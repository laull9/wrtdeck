...
| 覆盖低版本安装（旧配置自动迁移、用户数据与自定义项保留、升级钩子幂等） | 可用，附升级安装模拟 |
...
- **免登录**：从 LuCI 点进面板不需要再输一次口令。做法是 rpcd 用设备本机的 Token 换一张 60 秒有效、只能兑一次的登录码，父窗口用 `postMessage` 递给面板，面板兑成自己的会话凭据（默认存 `sessionStorage`，关掉标签页即失效）。长期凭据不出设备进程，地址栏与浏览器历史里也不留东西。
  - 另有一条「网关用 LuCI 登录状态代签凭据」的分支，但在原生 LuCI 上**不会生效**：LuCI 的会话 cookie 写了 `path=/cgi-bin/luci/`，浏览器不会把它发给网关所在的 `/cgi-bin/<cgi_prefix>/wrtdeck-api`。它是 fail-closed 的，只空转，不影响功能与安全。详见 `docs/ARCHITECTURE.md` 第 13 节。
