...
/etc/init.d/owdash password    # 重置登录口令（忘记口令时用）
```
...
`/etc/owdash/config.json`。它被声明为 conffiles，升级不会覆盖你改过的内容。
...
| `data_dir` | 注册表、密钥、配置与自签证书的落盘目录，改完要同步改 `/etc/init.d/owdash` 里的 `CONF_DIR` |
...
```json
{
  "tls": {
    "enabled": true,
    "cert_file": "/etc/owdash/tls.crt",
    "key_file": "/etc/owdash/tls.key",
...
```sh
/etc/init.d/owdash restart
logread -e owdash | grep -i fingerprint
```
...
```sh
apk add --allow-untrusted ./owdash-1.0.1-r1.apk          # apk 升级
opkg install ./owdash_1.0.1-1_aarch64_cortex-a53.ipk      # ipk 升级

apk del owdash              # apk 卸载
opkg remove owdash          # ipk 卸载
```

- 升级时 `/etc/owdash/config.json` 因 conffiles 声明而保留你的改动；控制脚本会 `restart` 服务而不是重新 `enable`。
...
```text
dist/owdash-<版本>-<发布号>.apk                    面板，按架构区分
dist/owdash_<版本>-<发布号>_<架构>.ipk             面板，按架构区分
...
- **apk** 是新版分段格式（多个 gzip 成员拼成的 tar 流 + 尾部签名），由 `scripts/apk_build.py` 生成并用内置 RSA 私钥签名，公钥同时输出到 `dist/owdash-local.rsa.pub`。
...
| 登录一直提示失败 | 可能已触发节流锁定，看 `logread -e owdash \| grep '\[auth\]'`；锁定按指数退避，最长 1 小时 |
| 忘了登录口令 | 在设备上执行 `/etc/init.d/owdash password` 重置 |
